    ...
    >
      <div className="max-w-6xl mx-auto flex items-center justify-between">
        ...
        <div className="md:hidden">
          <button onClick={() => setOpen((v: boolean) => !v)} aria-label="Toggle menu" className="p-2 rounded bg-blue-100 dark:bg-gray-800">
      ...
      {/* Mobile dropdown */}
      {open && (
        <div className="md:hidden absolute right-4 top-full mt-2 min-w-[10rem] bg-white dark:bg-gray-800 rounded shadow-lg z-50">
          <div className="flex flex-col p-2">
            <Link href="/" onClick={() => setOpen(false)} className="flex items-center justify-center px-4 py-2 rounded hover:bg-blue-100">首页</Link>
            <Link href="/about" onClick={() => setOpen(false)} className="flex items-center justify-center px-4 py-2 rounded hover:bg-blue-100">关于</Link>
            <Link href="/practice" onClick={() => setOpen(false)} className="flex items-center justify-center px-4 py-2 rounded hover:bg-blue-100">练习</Link>
            <Link href="/blog" onClick={() => setOpen(false)} className="flex items-center justify-center px-4 py-2 rounded hover:bg-blue-100">文章</Link>
            <Link href="/contact" onClick={() => setOpen(false)} className="flex items-center justify-center px-4 py-2 rounded hover:bg-blue-100">联系</Link>

            {session?.user ? (
              <div className="pt-2 border-t">
                <div className="text-sm mb-2 px-4 py-2 text-center">欢迎，{session.user.name ?? session.user.email}</div>
                <button onClick={async () => { setOpen(false); await signOut({ callbackUrl: pathname || '/', redirect: false }); try { router.refresh() } catch { window.location.href = pathname || '/' } }} className="w-full text-center px-4 py-2 rounded bg-red-600 hover:bg-red-700 text-white">退出</button>
              </div>
            ) : (
                <div className="pt-2 border-t flex flex-col space-y-2">
                <Link href={`/login?callbackUrl=${encodeURIComponent(pathname || '/')}`} onClick={() => setOpen(false)} className="flex items-center justify-center px-4 py-2 rounded bg-blue-700 text-white">登录</Link>
                <Link href="/register" onClick={() => setOpen(false)} className="flex items-center justify-center px-4 py-2 rounded bg-green-700 text-white">注册</Link>
              </div>
            )}
          </div>
        </div>
